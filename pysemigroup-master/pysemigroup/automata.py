...
class Automaton(object):
    ...
    def kleene_star(self):
        ...
        ninit = fresh(self._states)
        for a in self._alphabet:
            L = []
            for i in self._initial_states:
                L.extend(self._transitions.get((i, a), []))
            transitions[(ninit, a)] = list(set(L)) 
        for x, L in list(transitions.items()):
            if self._final_states.intersection(L):
                transitions[x].append(ninit)
        final_states = [ninit]
        return Automaton(transitions, [ninit], [ninit], alphabet=self._alphabet)
    ...
    def _minimal_automaton_hopcroft(self):
        ...
        final_states = []
        initial_states = [] 
        ini = next(iter(AD._initial_states))
        for i in states:
            if ini in i:
